# WaveLens demo runbook

## Before presenting

1. Launch `dist/WaveLens.app` on the presentation Mac. Confirm a camera image appears and the MacBook speakers or chosen output device are audible. If using an iPhone, connect Continuity Camera before opening WaveLens and check the macOS Video menu for Main camera, Zoom 1×, and Center Stage off.
...
4. If a completed ElevenLabs song is available in `cache/songs/`, keep its job JSON and MP3 together. WaveLens restores a completed song when the same arrangement is selected on the Collections page. The current workspace has no completed ElevenLabs MP3.
...
- **No camera:** check System Settings → Privacy & Security → Camera → WaveLens, then confirm the camera is available in macOS and reopen the app. WaveLens currently uses camera index `0`; device order can change when Continuity Camera connects.

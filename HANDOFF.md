...
1. 启动后是实时预览：画面是低饱和的灰调，被认出的物体有一个很淡的细线圆角框，左下角毛玻璃卡片写着 "On the desk" 和当前看到的物体。此时**没有声音**（空桌 = 安静，这是 Aaron 明确要的）。加载相机和模型的那几秒里也是静的（`Vision.loading`）：上次存的乐队虽然已经恢复到乐器架上，但走带闸门还关着（`Engine.transport`），加载完才自动打开，而且是从**第一小节第 0 步**开始，不会从小节中间接进去。等不及可以先按 `p` 或点乐器架，那样立刻起奏；起奏之后你自己按的暂停不会被任何东西解除。没给摄像头权限时不会自动起奏（一直停在等权限的画面），按 `p` 仍然可以。
2. 按空格（或点屏幕下方的圆形快门）：画面闪白并定格。程序对定格的这一帧再做一次全分辨率精识别。
3. 定格画面里，每个物体的框内恢复颜色（叠着这件乐器自己的颜色滤镜，和乐器架缩略图一样），框旁写着 `物体 · 乐器`，对应乐器开始演奏。某个乐器发声的瞬间，它的框会轻微放大、变亮（"呼吸"）。卡片标题变成 "Band"，右上角显示当前和弦；卡片里每行前的小圆点也是这件乐器的颜色，每出一个音亮一下。
...
| `tests/test_stage.py` | | 舞台：响度映射单调、复杂度中间不变/左稀右密/不跑调、随机落位不重叠不贴边、随机发牌的三条规则、位置存盘 |
...
每个乐器的 `detect=[...]` 是一组同义提示词，任何一个触发都算；`ALIASES` 把提示词映射回声部；`SHOW_AS` 决定屏幕上显示的名字（iPad 显示为 tablet）。同一声部的两个重叠框（IoU > 0.5）只保留分数高的（`vision.merge_duplicates`）。模型可评估多个候选，但界面和拍照每次只选一个；预览框使用平滑移动和短暂的目标保持，切换目标时不会同时显示旧框。拍照时还会把"最近 0.5 秒内见过但这一帧没认出来"的物体补进去，用来对抗闪烁。
...
- `mode`：`preview`（实时预览）、`show`（照片定格中）或 `summary`（收藏总览）。三种状态下乐队都可能在演奏，是否有声看 `parts`
...
- `playing`：演奏 / 暂停总开关的状态
- `math`：数学旋律模式是否打开
- `view`：当前显示 `camera`、`stage` 或 `summary`；`placed`：每件已保存、放过位置的乐器在舞台上的坐标（0–1，和 `place` 指令一样）
...
- PL 端拥有 100 MHz 主时钟、可编程 BPM/十六分音符时钟、七轨 16-step sequencer、事件 FIFO、envelope/LFO、16-bit maximal LFSR、七路无除法 Euclidean/Bresenham generator，以及四击 tap-tempo 计时器。发生器可在安全网格上新增节奏点；Mac 为新增点选择相邻的和弦安全音高，鼓使用轻 hi-hat。下拍保留，全部七轨（包括 bass/strings）都参与密度和节奏网格变化。
...
- RTL、AXI、固件协议和 Mac 协议测试均已通过；包含全七轨变化及 BTN3 长按复位的完整 Zybo implementation 在 100 MHz 下 timing/DRC 通过（setup WNS +0.043 ns、hold WHS +0.034 ns、0 routing errors），并已生成 4,213,904-byte `fpga/build/BOOT.BIN`。兼容新版 shelf UI 的固件不再在 BTN0/BTN1 上重复修改 transport/mask，因此拍照不会重启音乐小节。
...
- 新版 PL ID 是 `44420103`，`tools/zybo_smoke.py` 会检查 32 个 event：第一小节必须等于 base pattern，第二小节必须逐位等于主机镜像的 LFSR/Euclidean 安全网格结果，并继续检查 envelope/LFO。
- 当前仓库新版 `BOOT.BIN` SHA-256：`44d8e603b8ac3c4516b99f09784cfd5e7a92d547ec4638118e88e4f0f401cd12`。该镜像的 PL ID 为 `44420103`，尚未写入实体板。板上之前逐块回读验证的 QSPI 镜像为 PL ID `44420102`，不包含 bass/strings 变化。
- **剩余工作**：将新 `BOOT.BIN` 写入 QSPI 并验证；断电后确认 JP5 在 QSPI（不要带电移动），冷启动并通过新版 smoke test；在 Mac 上运行 WaveLens（会自动启动 `tools/zybo_bridge.py`），检查全七轨自动小节变化、两种按钮模式和长时间无 FIFO overflow。J12 板载 FT2232 已是 USB-UART，不需要 TTL 串口模块或网线。接线及命令见 `fpga/README.md`。

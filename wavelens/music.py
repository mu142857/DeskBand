...
class BarView:
    ...
    eighths: bool = False          # planned on the eighth-only grid
...
class Pattern:
    ...
    def eighths_only(self):
        """The eighth-only grid (the board's BTN2, `g` here): notes on the odd
        16ths are left out. Never silent: a bar made only of those stays."""
        kept = {s: notes for s, notes in self.bar.items() if s % 2 == 0}
        if kept:
            self.bar = kept
...
class Composer:
    def __init__(self, chords=None, motif_seeds=None):
        ...
        self.eighths = False           # rhythm grid: mixed 8ths and 16ths, or 8ths only
    ...
    def set_eighths(self, on=None):
        """Eighth-only rhythm grid on / off (None flips it), from the next bar line."""
        self.eighths = (not self.eighths) if on is None else bool(on)
    ...
    def rhythmic_events(self, name, step):
        ...
        if part is None or (self.bar_view.eighths and step % 2):
            return []
        return part.rhythmic_events(step % P)
    ...
    def step(self, step):
        ...
        if s == 0:
            ...
            for p in self.parts.values():
                ...
                if eighths:
                    p.eighths_only()
            self.bar_view = BarView(math, {n: tuple(sorted(p.bar)) for n, p in self.parts.items()}, eighths)

...
module tb_bar_variation_engine;
    ...
    initial begin
        ...
        if (generated_patterns[0] != 16'h1511 ||
            generated_patterns[1] != 16'h5555 ||
            generated_patterns[2] != 16'h1041 ||
            generated_patterns[3] != 16'h5151 ||
            generated_patterns[4] != 16'h0001 ||
            generated_patterns[5] != 16'h4444 ||
            generated_patterns[6] != 16'h4924)
            $fatal(1, "first generated bar changed from the reference model");
        for (int track = 0; track < NUM_TRACKS; track++) begin
            if ((generated_patterns[track] & ~base_patterns[track]) != 0)
                $fatal(1, "generator created a trigger outside the base pattern");
        ...
        // NORMAL -> FULL is queued and makes every unlocked track use all
        // valid events on the next bar.
        pulse_energy();
        ...
        for (int track = 0; track < NUM_TRACKS; track++) begin
            if (track != 3 && generated_patterns[track] != base_patterns[track])

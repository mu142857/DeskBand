...
the sound lives in Logic's library and on disk. Generated from wavelens/config.py."""
...
def info(voice):
    ...
    if voice == "keys":
        return dict(name="柔和电钢琴（程序合成，不用采样）", logic="无，代码在 wavelens/synth.py 里的 keys 音色",
                    source="—", used="—")
    if voice == "sax":
    ...
    if voice == "vocal":
        return dict(name="人声 Voices（ElevenLabs 生成的 \"ooh\" 长音）", logic="无，不来自 Logic",
                    source="ElevenLabs 音效生成接口，提示词在 wavelens/config.py 的 VOCAL_PROMPTS",
...
def main():
    ...
    for obj, spec in C.INSTRUMENTS.items():
        ...
        L.append(f"    摄像头认这些词：{' / '.join(spec['detect'])}" if spec["detect"] else
                 "    怎么认：MediaPipe 人脸关键点，最大的那张脸张着嘴拍照（wavelens/face.py）")
    ...
    L.append("然后把 wavelens/config.py 里的 CONCERT_GRAND（钢琴）或 KINGS_CROSS（弦乐）指到那个文件夹。")
    L.append("--velocity 是力度层：45 左右是弱奏（朦胧），84 左右是中强。")
    L.append("")
    L.append("三、效果")
    L.append("-" * 60)
    L.append(f"    大厅混响（wavelens/fx.py）：room {C.REVERB['room']}，damp {C.REVERB['damp']}，预延迟 {C.REVERB['predelay_ms']:.0f} ms")
    L.append(f"    钢琴低通：{C.PIANO_LOWPASS_HZ} Hz（调低更闷、更远；0 = 关）")
    on = lambda k: "开" if C.BACKING[k] else "关"
    L.append(f"    背景层：黑胶噪声 {on('vinyl')}，沙锤/rim {on('perc')}，低音铺底 {on('sub')}（开关在 wavelens/config.py 的 BACKING）")
    L.append("")

...
Keys: `space` shoot / retake · click a shelf thumbnail or `1`–`8` (counting from the top) switch a saved instrument on or off · `e` open the summary; `b`, `e`, or Escape returns (on the summary, `1`–`8` toggle its cards) · `p` or return play / pause · `m` math mode on / off · `tab` camera / stage · `r` deal the stage again (on the stage) · `0` deselect them all · right-click a slot (or hover and press `x`) forget it · `s` save the live frame to `cache/shots/` · `d` debug overlay · `f` fullscreen · `q` quit.

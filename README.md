...
- **Photo description (Gemini):** with `GEMINI_API_KEY` set or a private `cache/gemini_api_key` file, a captured object's photo is sent to Gemini (`GEMINI_MODEL` in `wavelens/config.py`) in the background. Its description appears under the title while the photo is shown and on the object's Collections card. Older saved items without descriptions are filled in from their saved thumbnails when Collections opens, one at a time. Hover over a card to read the full description. The first description an object is given stays, even when it is photographed again. It changes nothing in the music.
...
Put a working key in the ignored `cache/elevenlabs_api_key` file when you are ready; this project has no `local.env` file or parser. Keep the key file readable only by your user (`chmod 600 cache/elevenlabs_api_key`). `tools/run_app.sh` and the rebuilt `WaveLens.app` load it into the app process; no system environment change is needed. An existing `ELEVENLABS_API_KEY` process variable takes precedence. For Gemini, use `GEMINI_API_KEY` or place the key in ignored `cache/gemini_api_key` with `chmod 600 cache/gemini_api_key`; the desktop app reads that private file directly.
